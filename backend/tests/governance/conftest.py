"""Pytest fixtures for the Governance Intelligence Center test suite (E08).

Mirrors tests/risk/conftest.py's role/TestClient fixtures — no live
connector round-trip is needed here (governance metadata is authored
directly, not derived from a live connection), so no real SQLite files
are required.
"""
...
@pytest.fixture()
def connection(db):
    conn = DBConnection(name="Gov Conn", type="sqlite", config={"path": "/tmp/gov.db"})
    db.add(conn)
    db.commit()
    db.refresh(conn)
    return conn
