"""Pytest fixtures for the Data Quality Observatory test suite (E06).

Mirrors tests/risk/conftest.py's role/TestClient fixtures — no live
connector round-trip is needed here (scorecards read persisted
ColumnProfile rows only), so no real SQLite files are required.
"""
...
@pytest.fixture()
def connection(db):
    conn = DBConnection(name="DQ Conn", type="sqlite", config={"path": "/tmp/dq.db"})
    db.add(conn)
    db.commit()
    db.refresh(conn)
    return conn
