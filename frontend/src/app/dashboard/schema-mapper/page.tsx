      ...
      api.get<{ tables: Array<{ id: number; table_name: string; short_name: string; columns: any[] }> }>(
        ...
        .then(res => {
          ...
          if (formattedTables.length > 0) {
            ...
            setExpandedNodes({ [`table:${formattedTables[0].id}`]: true });
          }
        })
    ...
    api.get<{ schema: Record<string, SparkColumn[]> }>(`/api/v1/connectors/${targetConnId}/schema`)
      .then(res => {
        ...
        if (formatted.length > 0) {
          ...
          setExpandedNodes({ [`table:${formatted[0].id}`]: true });
        } else {
            ...
            {loading ? (
              ...
              Object.entries(hierarchy).map(([catalogName, databases]) => (
                <div key={catalogName} className="mb-2">
                  {/* Catalog Node */}
                  <div onClick={() => toggleNode(`catalog:${catalogName}`)} className="flex items-center gap-2 text-white/70 mb-1 cursor-pointer hover:text-white transition-colors p-1 rounded">
                    <span className="text-[10px] w-3 text-center">{expandedNodes[`catalog:${catalogName}`] ? "▼" : "▶"}</span>
                    ...
                    <span className="truncate font-semibold text-white/90">{catalogName}</span>
                  </div>

                  {/* Databases */}
                  {expandedNodes[`catalog:${catalogName}`] && (
                    <div className="ml-3 pl-2 border-l border-white/[0.06] mb-1">
                      {Object.entries(databases as Record<string, any[]>).map(([dbName, dbTables]) => (
                        <div key={dbName} className="mb-1">
                          {/* Database Node */}
                          <div onClick={() => toggleNode(`db:${catalogName}.${dbName}`)} className="flex items-center gap-2 text-white/70 mb-1 cursor-pointer hover:text-white transition-colors p-1 rounded">
                            ...
                            <div className="ml-3 pl-2 border-l border-white/[0.06]">
                              {dbTables.map((t: any) => (
                                <div key={t.id} className="mb-1">
                                  {/* Table Node */}
                                  <div onClick={() => { toggleNode(`table:${t.id}`); setSelectedTable(t); }} className={["flex items-center gap-2 text-white/70 mb-1 cursor-pointer hover:text-white transition-colors p-1 rounded", selectedTable?.id === t.id ? "bg-white/10" : ""].join(" ")}>
                                    <span className="text-[10px] w-3 text-center">{expandedNodes[`table:${t.id}`] ? "▼" : "▶"}</span>
                                  ...
                                  {/* Columns */}
                                  {expandedNodes[`table:${t.id}`] && (
                                    <div className="ml-4 pl-2 border-l border-white/[0.06] space-y-1.5 mb-1">
                                      {t.columns.map((col: any) => (
                                        <div key={col.id} className="flex items-center justify-between px-2 py-1.5 rounded transition-colors hover:bg-white/[0.04]">
                                          <div className="flex items-center gap-2">
